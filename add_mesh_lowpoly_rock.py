...
bl_info = {
    "name": "LowPoly Rock",
    "author": "Punnawut Khowkittipaiboon, Hidesato Ikeya",
    "version": (0, 2, 0),
    "blender": (2, 80, 0),
...
OPERATOR_TEXT = "LowPoly Rock"
ORIGIN_NAME = ROCK_NAME + "DisplaceOrigin"
TEXTURE_NAME = ROCK_NAME + "Texture"
TEMP_MESH_BASENAME = "tempmesh"
ANGLE_MAX = radians(90)

# Custom type hints
Dummy = Tuple[float, float, float]

def get_basemesh(context: bpy.types.Context, subdiv: int = 5, radius: float = 1.0, ratio: Vector = (1., 1., 1.)):
    me = context.blend_data.meshes.new(TEMP_MESH_BASENAME + str(uuid4()))
    bm = bmesh.new()
    bm.from_mesh(me)
    mat = Matrix()
...
def get_texture(context: bpy.types.Context, name: str, size: float = 1.0, brightness: float = .8, contrast: float = .8, weights: Vector = (1.0, .3, .0)):
...
def create_rock(context: bpy.types.Context, subdiv: int, radius: float, size_ratio: tuple,
                noise_center: Vector, noise_size: float, noise_brightness: float,
                sharpness: float, displace_midlevel: float, displace_strength: float,
                voronoi_weights: Vector, simplicity: float, collapse_ratio: float):
...
class MESH_OT_LowPolyRock(bpy.types.Operator):
    """LowPoly Rock"""
    ...
    def execute(self, context: bpy.types.Context):
        ...
        for n in range(self.num_rock):
            ...
            if self.keep_modifiers:
                ...
                if not rock.select_get():
                    rock.select_set(True)
                        
                for modifier in rock.modifiers:
...
def add_rock_button(self: bpy.types.VIEW3D_MT_mesh_add, context: bpy.types.Context):
    self.layout.operator_context = 'INVOKE_DEFAULT'
    self.layout.operator(MESH_OT_LowPolyRock.bl_idname, text=OPERATOR_TEXT, icon="PLUGIN")

def register():
    bpy.utils.register_class(MESH_OT_LowPolyRock)
    bpy.types.VIEW3D_MT_mesh_add.append(add_rock_button)

def unregister():
    bpy.utils.unregister_class(MESH_OT_LowPolyRock)
    bpy.types.VIEW3D_MT_mesh_add.remove(add_rock_button)
